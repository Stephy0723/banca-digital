...
import { ArrowLeftRight, ArrowUpRight, ArrowDownLeft, CheckCircle } from 'lucide-react';
import './Transfers.css';

const recentContacts = [
  { name: 'Juan Pérez', bank: 'Banco Popular', color: '#2da248', lastAmount: 15000 },
  { name: 'María López', bank: 'Banreservas', color: '#1a4a6e', lastAmount: 25000 },
  { name: 'Carlos Reyes', bank: 'BHD León', color: '#0b2a41', lastAmount: 8000 },
  { name: 'Ana García', bank: 'CoopEocala', color: '#a0ce4e', lastAmount: 12000 },
];
    ...
    <div className="page-container transfers">
      ...
      <div className="transfers__grid">
        <form className="card transfers__form-card" onSubmit={handleSubmit}>
          <div className="transfers__form-title">
            <ArrowLeftRight size={20} /> Nueva Transferencia
          </div>

          <div className="transfers__form-group">
            <label className="transfers__form-label">Tipo de transferencia</label>
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="button" className={`btn ${toType === 'own' ? 'btn-primary' : 'btn-secondary'}`} style={{ flex: 1 }} onClick={() => setToType('own')}>
                Entre mis cuentas
              </button>
              <button type="button" className={`btn ${toType === 'third' ? 'btn-primary' : 'btn-secondary'}`} style={{ flex: 1 }} onClick={() => setToType('third')}>
                A terceros
              </button>
            </div>
          </div>
          ...
          <div className="transfers__form-group">
            ...
            <select className="transfers__select" value={fromAccount} onChange={e => setFromAccount(e.target.value)}>
          ...
          {toType === 'own' ? (
            <div className="transfers__form-group">
              ...
              <select className="transfers__select" value={toAccount} onChange={e => setToAccount(e.target.value)}>
            ...
            <>
              <div className="transfers__form-group">
                ...
                <input className="input" placeholder="Nombre del beneficiario" value={beneficiary} onChange={e => setBeneficiary(e.target.value)} />
              </div>
              <div className="transfers__form-group">
                <label className="transfers__form-label">Cuenta destino</label>
                <input className="input" placeholder="Número de cuenta" value={toAccount} onChange={e => setToAccount(e.target.value)} />
              </div>
            </>
          )}
          ...
          <div className="transfers__form-group">
            ...
            <div className="transfers__amount-wrapper">
              ...
              <input className="input transfers__amount-input" type="number" placeholder="0.00" min="0" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} />
            </div>
          </div>

          <div className="transfers__form-group">
            <label className="transfers__form-label">Concepto (opcional)</label>
            <input className="input" placeholder="Descripción de la transferencia" value={concept} onChange={e => setConcept(e.target.value)} />
          </div>
          ...
          {amount && parseFloat(amount) > 0 && (
            <div className="transfers__summary">
              ...
              <div className="transfers__summary-row transfers__summary-total">
                ...
                <span className="transfers__summary-value" style={{ color: 'var(--color-accent)' }}>{formatCurrency(parseFloat(amount))}</span>
              </div>
              {selectedFrom && (
                <div className="transfers__summary-row">
                  <span className="transfers__summary-label">Balance después</span>
                  <span className="transfers__summary-value">{formatCurrency(selectedFrom.balance - parseFloat(amount))}</span>
                </div>
              )}
            </div>
          )}

          <button type="submit" className="btn btn-accent transfers__submit">Confirmar Transferencia</button>
        </form>
        ...
        <div className="card transfers__recent">
          ...
          <div className="transfers__contact-list">
            {recentContacts.map(contact => (
              <div key={contact.name} className="transfers__contact" onClick={() => setBeneficiary(contact.name)}>
                ...
                <div className="transfers__contact-amount">{formatCurrency(contact.lastAmount)}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-tertiary)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {t.type === 'sent' ? <ArrowUpRight size={14} style={{ color: 'var(--color-error)' }} /> : <ArrowDownLeft size={14} style={{ color: 'var(--color-accent)' }} />}
                  <div>
                    <div style={{ fontSize: '0.84rem', fontWeight: 600, color: 'var(--text-primary)' }}>{t.name}</div>
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-tertiary)' }}>{t.date}</div>
                  </div>
                </div>
                <div style={{ fontSize: '0.875rem', fontWeight: 700, color: t.type === 'received' ? 'var(--color-accent)' : 'var(--color-error)' }}>
      ...
      {showSuccess && (
        <div className="transfers__modal-overlay" onClick={handleClose}>
          <div className="transfers__modal" onClick={e => e.stopPropagation()}>
            <div className="transfers__modal-icon"><CheckCircle size={48} style={{ color: 'var(--color-accent)' }} /></div>
